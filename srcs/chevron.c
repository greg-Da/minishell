...
/*   Updated: 2025/06/02 13:42:24 by greg             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../include/minishell.h"

static int parse_chevron_type(char **tmp, char chevron, t_parser *info, int *append)
{
...
int process_chevrons(char **pipes, int i, int fd[2], t_parser *info)
{
    ...
    if (!tmp)
        return (-1);
    char *start = tmp;
    char *next_chevron = get_next_chevron(tmp);

    while (next_chevron)
    {
        char chevron = *next_chevron;
        tmp = next_chevron + 1;
        int append = 0;
        parse_chevron_type(&tmp, chevron, info, &append);

        next_chevron = get_next_chevron(tmp);
        char *filename = extract_filename(tmp, next_chevron);
        if (!filename)
            return (free(start), -1);
        ...
        int *current_fd = (chevron == '<') ? &fd[0] : &fd[1];
