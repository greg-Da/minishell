...
/*   Updated: 2025/06/18 11:42:13 by qbaret           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	**build_new_env(char **envp, char *key)
{
	int		i = 0, j = 0, len = ft_strlen(key), skip;
	char	**new_env;
	int		env_count = count_env_vars(envp);
	...
	while (envp[i])
	{
		skip = (ft_strncmp(envp[i], key, len) == 0
...
static int	unset_key(t_minish *manager, char *arg)
{
	...
	if (!key)
		return (1);
	new_env = build_new_env(manager->envp, key);
	free(key);
	if (!new_env)
		return (1);
	free(manager->envp);
